...
for arg in "$@"; do
  case $arg in
    ...
    --no-dry)
      ...
      ;;
    *)
      echo "Usage: $0 [-n|--dry|-d] [--no-dry]"
      echo "  -n, --dry, -d: Run in dry-run mode (default)"
      echo "  --no-dry: Perform actual sync operations"
      exit 1
      ;;
  esac
done
...
main() {
  ...
  if [ "$DRY_RUN" = true ]; then
    ...
    log_info "Run with --no-dry to perform actual sync operations"
  fi
}
